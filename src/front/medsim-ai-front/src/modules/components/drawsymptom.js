...
function SymptomVisualizer({ coordinatesData = [] }) {
  ...
  const DefaultPannel = ({ symptomData, setSymptomsData }) => {
    ...
    const calculateBMI = () => {
      const heightInM = parseFloat(patientInfo.height) / 100; // Convert cm to m
    ...
    const renderField = (label, field, tooltip = null) => {
      return (
        <div className="space-y-1 relative group">
          <p className="text-blue-100 text-sm flex items-center">
            ...
            {tooltip && (
              <span className="ml-1 cursor-help relative">
                ...
                <span
                  className="hidden group-hover:block absolute z-10 bottom-full left-1/2 transform -translate-x-1/2 mb-2 p-2 w-48 bg-gray-800 text-xs text-white rounded shadow-lg whitespace-pre-line"
                >
                  {tooltip}
                </span>
          ...
          {editMode ? (
            ...
            <div
              className="w-full p-1 bg-gray-700 border border-gray-600 rounded text-xs text-white"
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="w-full sm:w-2/3 bg-gray-400 bg-opacity-30 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    {renderField('BMI Type', 'bmiType', 'Healthy: BMI due to healthy methods like exercise\nUnhealthy: BMI due to unhealthy methods like eating junk food')}
                  </div>

                  {/* BMI Calculation Display */}
                  <div className="mt-4 border-t border-gray-600 pt-3">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300 text-sm">Body Mass Index (BMI):</span>
